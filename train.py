...
try:
    ...
    print(f"Loaded dataset with {len(label_map)} classes")
except:
    print("Error: Please run the improved load_preprocess.py first!")
    exit(1)
...
model = OptimizedSymbolCNN(num_classes=num_classes, dropout_rate=0.4).to(device)
...
optimizer = optim.AdamW(model.parameters(), lr=0.002, weight_decay=0.01)  # Higher LR + weight decay

# Learning rate scheduler - more sophisticated
...
for epoch in range(num_epochs):
    ...
    if patience_counter >= patience and epoch > 30:  # Don't stop too early
...
def plot_training_curves():
    ...
    # Class-wise accuracy
    class_accs = []
    for i in range(min(20, len(unique_classes))):
...
if test_acc >= 95.0:
    ...
    print("💡 Try increasing epochs, adjusting learning rate, or adding more data augmentation")
